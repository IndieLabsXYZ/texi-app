...
// Optimize Images
function images() {
  return src('src/img/*.{jpg,png,gif,svg}', { allowEmpty: true })
    .pipe(plumber())
    .pipe(gulpIf(hasFiles('src/img/*.{jpg,png,gif,svg}'), imagemin([
...
function copy() {
  return src('src/**/*.{json,ico,woff,woff2,md,txt}', { base: 'src', allowEmpty: true })
    .pipe(plumber())
    .pipe(dest('dist'));
}
...
function watchFiles() {
  ...
  watch('src/**/*.css', css);
  watch('src/**/*.js', js);
  watch('src/img/*.{jpg,png,gif,svg}', images);
  watch('src/**/*.{json,ico,woff,woff2}', copy);
}

// Define tasks
export { clean, html, css, js, images, copy, serviceWorker };
export const build = series(clean, parallel(html, css, js, images, copy), serviceWorker);
